...
class OpenWeatherMenuButton extends PanelMenu.Button {
    ...
    _init(metadata, settings) {
        console.log(`enabling ${settings.get_string('clock-format')}`);
        ...
        if (Main.panel._menus === undefined) Main.panel.menuManager.addMenu(this.menu);
    ...
    loadConfig() {
        if (this._cities.length === 0) this._cities = '43.6534817,-79.3839347>Toronto >0';
        ...
        if (this.locale.indexOf('_') != -1) this.locale = this.locale.split('_')[0];
    ...
    get _actual_city() {
        var a = this.settings.get_int('actual-city');
        var b = a;
    ...
    _onPreferencesActivate() {
        this.menu.close();
        Extension.lookupByUUID('openweather-extension@jenslody.de').openPreferences();
        return 0;
    }
    ...
    recalcLayout() {
        ...
        if (
            ...
            this._forecastScrollBox.get_hscroll_bar().show();
    ...
    toBeaufort(w, t) {
        ...
        else if (w > 1.5 && w <= 3.4) return !t ? '2' : '(' + _('Light breeze') + ')';
        else if (w > 3.4 && w <= 5.4) return !t ? '3' : '(' + _('Gentle breeze') + ')';
        else if ((w > 5, 4 && w <= 7.9))
            return !t ? '4' : '(' + _('Moderate breeze') + ')';
        else if (w > 7.9 && w <= 10.7) return !t ? '5' : '(' + _('Fresh breeze') + ')';
        else if (w > 10.7 && w <= 13.8) return !t ? '6' : '(' + _('Strong breeze') + ')';
        else if (w > 13.8 && w <= 17.1) return !t ? '7' : '(' + _('Moderate gale') + ')';
        else if (w > 17.1 && w <= 20.7) return !t ? '8' : '(' + _('Fresh gale') + ')';
        else if (w > 20.7 && w <= 24.4) return !t ? '9' : '(' + _('Strong gale') + ')';
        else if (w > 24.4 && w <= 28.4) return !t ? '10' : '(' + _('Storm') + ')';
        else if (w > 28.4 && w <= 32.6) return !t ? '11' : '(' + _('Violent storm') + ')';
    ...
    formatTemperature(temperature) {
        ...
        return (
            parseFloat(temperature).toLocaleString(this.locale).replace('-', '\u2212') +
            ' ' +
    ...
    rebuildCurrentWeatherUi() {
        ...
        if (!this._isForecastDisabled) this._currentForecast.actor.destroy_all_children();
    ...
    scrollForecastBy(delta) {
        ...
        this._forecastScrollBox.get_hscroll_bar().adjustment.value += delta;
    }
    ...
    rebuildFutureWeatherUi(cnt) {
        ...
        pan.connect('pan', (action) => {
            ...
            this.scrollForecastBy(
                -1 *
                    ...
                    this._forecastScrollBox.get_hscroll_bar().adjustment.page_size
            );
            return false;
        });
        this._forecastScrollBox.add_action(pan);
        this._forecastScrollBox.connect('scroll-event', this._onScroll.bind(this));
        this._forecastScrollBox
            .get_hscroll_bar()
            .connect('scroll-event', this._onScroll.bind(this));
        this._forecastScrollBox.get_hscroll_bar().margin_right = 25;
        this._forecastScrollBox.get_hscroll_bar().margin_left = 25;
        this._forecastScrollBox.get_hscroll_bar().hide();
        ...
        if (cnt === 1) this._forecastExpander.label.set_text(_("Tomorrow's Forecast"));
        else this._forecastExpander.label.set_text(_('%s Day Forecast').format(cnt));
    ...
    _onScroll(actor, event) {
        ...
        this.scrollForecastBy(
            dy * this._forecastScrollBox.get_hscroll_bar().adjustment.stepIncrement
        );
        return false;
    }
}
