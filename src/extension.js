...
class OpenWeatherMenuButton extends PanelMenu.Button {
  ...
  loadConfigInterface() {
    this._settingsInterfaceC = this.settings.connect(
      "changed",
      () => {
        this.rebuildCurrentWeatherUi();
        this.rebuildFutureWeatherUi();
  ...
  rebuildFutureWeatherUi(cnt) {
    ...
    this._forecastScrollBox.get_hscroll_bar().connect(
      "scroll-event",
...
export default class OpenWeatherExtension extends Extension {
  enable() {
    this.openWeatherMenu = new OpenWeatherMenuButton(this.metadata, this.getSettings());
